...
- `__version__/0`, `__versioned__?/0`, and `resolve_version/1` generated by `use Hephaestus.Workflow` macro for all workflows.
- `:version` option for `use Hephaestus.Workflow` to set a custom workflow version (default: `1`).
- Umbrella module support via `:versions` and `:current` options in `use Hephaestus.Workflow`. Umbrella modules generate `__versions__/0`, `current_version/0`, `resolve_version/1`, and overridable `version_for/2` without requiring `start/0` or `transit/3`.
